...
import { canRespond, validateResponse } from "../src/domain/participant/eligibility";
...
const survey: Survey = { id: "team", title: "Team pulse", description: "A useful question", eligibility: "Team members", startsAt: "2026-09-01T00:00:00Z", endsAt: "2026-09-10T00:00:00Z" };
...
test("an eligibility result is bound to its own survey and open window", async () => {
  ...
  assert.equal(canRespond(survey, result, Date.parse(survey.startsAt) - 1), false);
...
test("private response flow rejects whitespace, oversize input and late submission", async () => {
  const result = await verifyDevelopmentEligibility(survey.id, "eligible");
  assert.equal(validateResponse(survey, result, "More quiet spaces, please.", now), null);
  assert.match(validateResponse(survey, result, "  ", now)!, /between/);
  assert.match(validateResponse(survey, result, "a".repeat(2001), now)!, /between/);
  assert.match(validateResponse(survey, result, "Valid response", Date.parse(survey.endsAt))!, /open/);
  assert.match(validateResponse(survey, null, "Valid response", now)!, /verified/);
});
